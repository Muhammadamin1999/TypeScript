...
let num2: number = 16 // it is explicit
let str: string = "hello world"
let isTrue: boolean = true // only true or false allowed
...
// it is explicit
function add1(c: number,d: number): number{
    return c + d;
}
...
const names: string[] = ['a','s','d'];
const user: {name: string, age: number} = {name: "li", age: 24};
